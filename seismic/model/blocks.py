...
class ConvBlock(nn.Module):
    def __init__(self, in_channels, out_channels, k_size=3, stride=1, padding=1, act=True):
...
#        x = self.batch_norm(self.conv3d(x))
        ...
        if self.act:
            ...
            pass
 #           print('Last Layer... No activation')
        ...
        for depth in range(model_depth - 2, -1, -1):
            ...
            for i in range(self.num_conv_blocks):
                if i == 0:
                    ...
                    self.module_dict["conv_{}_{}".format(depth, i)] = self.conv
            if depth == 0:
                self.final_conv = ConvBlock(in_channels=feat_map_channels * 2, out_channels=out_channels, act=False)
